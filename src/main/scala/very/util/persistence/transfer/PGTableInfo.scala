...
import very.util.persistence.transfer.wrapper.ResultSetIterator
...
object PGTableInfo {
  ...
  def getTableInfo(
    ...
    tableNameResult.flatMap { tableNames =>
      tableNames.map { tableName =>
        val result = Using(connection.prepareStatement(s"""SELECT
             |    f.attnum AS number,  
             |    f.attname AS name,  
             |    f.attnum,  
          ...
          val columns = ResultSetIterator(ps.executeQuery()).map { v =>
            ...
            Column2(columnName, dataType, !isNull, isAutoIncrement) -> v
              .boolean("primarykey")
          }.toList
