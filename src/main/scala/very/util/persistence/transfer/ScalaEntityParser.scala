...
object ScalaEntityParser {
  case class ColumnInScala(underlying: Column) {
    ...
    lazy val rawTypeInScala: String = underlying.dataType match {
      ...
      case JavaSqlTypes.TIMESTAMP => TypeName.Any //config.dateTimeClass.simpleName
  ...
  def fromTable(table:Table,
                nameF:String => String = toCamelCase.andThen(quoteReservedWord),
                columnNamePF:PartialFunction[String,String],
                fieldTypePF: PartialFunction[String, String],
                `extends`: List[String],
                `package`: String,
               ) = {
    val extendsFix = `extends`.map(extractImportsType)
    var imports = extendsFix.collect{
      case (Some(v),_ ) => v
    }
    val fields = table.allColumns.map{column =>
      val columNameTransfer= lowerCamelCase.andThen(quoteReservedWord)
      val newName = if(columnNamePF.isDefinedAt(column.name)) columnNamePF(column.name) else columNameTransfer(column.name)
      ...
      val fieldType = if(fieldTypePF.isDefinedAt(newName)) {
        ...
        hasImport.foreach(v => imports = imports:+v)
        if(!column.isNotNull) {
          s"Option[${typeName}]"
        } else {
          typeName
...
def toProperCase(s: String): String = {
  ...
  else s.substring(0, 1).toUpperCase(Locale.ENGLISH) + s.substring(1).toLowerCase(Locale.ENGLISH)
}
